...
from keyboards.default.user_menu import user_product_menu, user_main_menu, user_order_menu, user_stickers_menu_def
...
@dp.message_handler(text="👨‍💻 Coding")
async def coding_sticker_handler(message: types.Message, state: FSMContext):
    data = await state.get_data()
    basket = data.get('basket') if data.get('basket') else dict()
    item = basket.get("Coding") if basket.get("Coding") else dict()
    quantity = item.get('quantity') if item.get('quantity') else 0
    total = item.get('total') if item.get('total') else 0
    await state.update_data({
        "product": "Coding",
    ...
    photo = "https://images.uzum.uz/chp4ghlenntd8rfa8lrg/original.jpg"
    text = "👨‍💻 Coding\nDasturlash uchun stikerlar Isekai IT, Geek, Coding, 5-10 dona\nNarxi: 15000"
    await message.answer_photo(photo=photo, caption=text, reply_markup=await user_product_buy_def(quantity, total))



@dp.message_handler(text="🎲 Game")
async def game_sticker_handler(message: types.Message, state: FSMContext):
    data = await state.get_data()
    basket = data.get('basket') if data.get('basket') else dict()
    item = basket.get("Game") if basket.get("Game") else dict()
    quantity = item.get('quantity') if item.get('quantity') else 0
    total = item.get('total') if item.get('total') else 0
    await state.update_data({
        "product": "Game",
    ...
    photo = "https://images.uzum.uz/chp4ghlenntd8rfa8lrg/original.jpg"
    text = "🎲 Game\nStikerlar 'Genshin Impact', 6 turdagi, 50 dona\nNarxi: 25000"
    await message.answer_photo(photo=photo, caption=text, reply_markup=await user_product_buy_def(quantity, total))


@dp.message_handler(text="📞 Contact")
async def stickers_menu_handler(message: types.Message):
    photo = "https://honeywell.scene7.com/is/image/honeywell/contact-us_help-support-1?wid=1000&hei=668&dpr=off"
    text = "Admin: @akbarovv_sh\nPhone Number: +998950025424\nEmail: akbarov.exe@gmail.com"
...
async def show_product_handler(call: CallbackQuery, state: FSMContext):
    ...
    if basket:
        ...
        text = "Sizning savatingizda hech narsa mavjud emas ❗"
        await call.answer(text=text, show_alert=True)
